...
export default class BookInfo {
  ...
  public static createS3HarvesterLinkBase(book: any): string {
    ...
    const safeUploader =
      book.uploader && book.uploader.username
  ...
  public static createS3LinkBase(book: any, bucket: string): string {
    const safeUploader =
      book.uploader && book.uploader.username
        ? BookInfo.MakeUrlSafe(book.uploader.username)
        : "UNKNOWN";
    return `https://s3.amazonaws.com/${bucket}/${safeUploader}/${book.bookInstanceId}`;
  }
