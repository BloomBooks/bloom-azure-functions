...
export default class BloomParseServer {
  ...
  public static bookMatchesAtLeastOneFilter(
    ...
    // But for now, these two (tag and brandingProjectName) should handle all the cases we need.
    // In addition, by doing these two simple checks, we avoid another parse query because we're
    // just taking a simple look at the information we already have in the bookInfo object.
  ...
  public static async canModifyBook(userInfo, bookInfo) {
    ...
    // const bookInfo = await BloomParseServer.getBookInfo("w0DfxIRK15");//prod
    //const bookInfo = await BloomParseServer.getBookInfo("iCVdT3YkWT"); //dev eeaddab4-fbdd-4c50-8967-e7f9401fd657
