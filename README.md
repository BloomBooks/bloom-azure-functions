...
The URL used to access the function always ends with *opds* possibly followed by a query and then by optional parameters.  The recognized queries are

- **?main** - (default if no query or unrecognized query) Return the top-level OPDS page pointing to the ePUB and
"all" pages.
- **?epub** - Return a page which lists only entries that have a visible ePUB file to download and which
shows links only to epub artifacts.
- **?all** - Return a page listing all visible entries (for the desired language) whether or not they have any
...
- **&src**=XXX - (default value is *dev*, will become *prod*) specify the source parse table.  Specify the source
of the book information.  Possible values are
...
`http://localhost:7071/api/opds?epub&lang=fr&src=dev`
...
`https://localhost:7071/api/opds?epub&src=dev`
...
generated OPDS pages.  Any books that have a value set for *internetLimits.downloadShell.countryCode* will be
omitted from any of the generated OPDS pages.  (This latter check may be overly restrictive, but is certainly safe
legally.)

For the *?epub* OPDS pages, books whose epub artifact is set invisible by the *show* object from the *books* table
will be omitted.  Only entries whose epub is in the desired language are shown (to the best of our ability to
determine this).

All books will be shown in the *?all"* OPDS pages, but links to artifacts will be omitted if the *show* object
makes them invisible.  (In the *?all* OPDS pages, books may have an entry without any artifact links, although we
...
- *OpdsParseAppIdDev* - the AppId key to the development parse table (for *&src=dev* in the input URL, the default for the alpha stage of initial development)
- *OpdsParseAppIdProd* -  the AppId key to the production parse table (for *&src=prod* in the input URL, the default after the alpha stage of initial development)
