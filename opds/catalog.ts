...
export default class Catalog {
  ...
  public static async getCatalog(
    ...
    Catalog.Source = params["src"];
    if (!Catalog.Source) Catalog.Source = this.DefaultSource;
    switch (Catalog.Source.toLowerCase()) {
    ...
    switch (catalogType) {
      case CatalogType.EPUB:
        title = "BloomLibrary ePUB Books";
        break;
      case CatalogType.ALL:
        title = "BloomLibrary ePUB, PDF, and BloomPub Books";
        break;
      default:
        title = "BloomLibrary Books";
        break;
    }
...
`
  ...
  <entry>
    ...
    <link rel="subsection" href="${
      this.RootUrl
    }?epub${paramString}" type="application/atom+xml;profile=opds-catalog;kind=acquisition"/>
  </entry>
  <entry>
    ...
    <link rel="subsection" href="${
      Catalog.RootUrl
    }?all${paramString}" type="application/atom+xml;profile=opds-catalog;kind=acquisition"/>
  </entry>
`;
    /* eslint-enable indent */
  }
  ...
  private static GetParamsForUrl(): string {
    return `${this.DesiredLang === "en" ? "" : "&lang=" + this.DesiredLang}${
            ...
            .map(lang => {
              ...
              link =
                ...
                `  <link rel="http://opds-spec.org/facet" href="${
                  this.RootUrl
                }?${catalogType === CatalogType.EPUB ? "epub" : "all"}&lang=${
                  lang.isoCode
                }${
