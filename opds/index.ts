...
function getEntryFromBook(b:any) : string {
  let entry : string;
  entry = `
    <id>${b.bookInstanceId}</id>
    <title>${b.title}</title>
`;
  if (b.authors && b.authors.length > 0) {
    b.authors.map(a => {
      entry = entry + `    <author><name>a</name></author>
`;
    });
  } else {
...
`;
  entry = entry + `    <published>${b.createdAt}</published>
    <updated>${b.updatedAt}</updated>
`;
  if (b.publisher) {
    entry = entry + `    <dcterms:publisher>${b.publisher}</dcterms:publisher>
`;
  } else {
...
`;
  ...
  if (b.copyright) {
    entry = entry + `    <rights>${b.copyright}</rights>
`;
  } else {
...
`;
  ...
  if (b.license) {
    entry = entry + `    <dcterms:license>${b.license}</dcterms:license>
`;
  } else {
    entry = entry + `    <dcterms:license>UNKNOWN</dcterms:license>
`;
  }
  if (b.langPointers && b.langPointers.length > 0) {
    ...
    b.langPointers.map(lang => {
      entry = entry + `    <dcterms:language>${lang.isoCode}</dcterms:language>
`;
    });
  }
  else if (b.languages && b.languages.length > 0) {
...
`;
  ...
  const baseUrl = b.baseUrl.replace(/%2f/g, "/"); // I don't know why anyone thinks / needs to be url-encoded.
  ...
  const safeUploader = b.uploader ? MakeUrlSafe(b.uploader.username) : "UNKNOWN";
  const pdfLink = baseUrl + name + ".pdf";
  const epubLink = harvestHead + safeUploader + "/" + b.bookInstanceId + "/epub/" + name + ".epub";
  const bloomdLink = harvestHead + safeUploader + "/" + b.bookInstanceId + "/" + name + ".bloomd";
