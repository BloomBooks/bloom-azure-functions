...
export default class Books {
  private static getParseUrl(tableName: string): string {
    switch (Catalog.Source) {
      ...
      default:
        return (
          "https://bloom-parse-server-develop.azurewebsites.net/parse/classes/" +
          tableName
        );
      case CatalogSource.PRODUCTION:
        return (
          "https://bloom-parse-server-production.azurewebsites.net/parse/classes/" +
          tableName
        );
    }
  }
      ...
      axios
        .get(this.getParseUrl("books"), {
          ...
          params: {
            ...
            limit: 100000, // is there a default limit?
            //skip: 100,
            order: "title",
  ...
  public static getEntryFromBook(
    ...
    // filter on country restrictions
    // REVIEW: IS THIS THE RIGHT APPROACH?  OR SHOULD WE IGNORE THIS FOR OPDS LISTING?
    if (
      book.internetLimits &&
      ...
      // always be omitted from public catalogs.
      return entry;
    }
...
`
  ...
  private static getLinkFields(book: any, catalogType: CatalogType) {
    ...
    if (catalogType === CatalogType.EPUB) {
      // already checked book.show.epub
      const epubLink =
