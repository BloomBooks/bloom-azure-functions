...
export async function longRunningUploadStart(
  ...
  if (isNewBook) {
    ...
    // In other words, if there are timestamps for our book ID which don't match the current baseUrl,
    // delete them.
    // We will create a new uploadPendingTimestamp below which will be used for the new set of files.
    if (existingBookInfo.uploadPendingTimestamp) {
      try {
        await deleteBook(bookObjectId, env, existingBookPath);
      } catch (err) {
        return handleError(
          500,
